...
int main()
{
    ...
    int result = binarySearch(array, 0, size-1, search);            //here left index for binary search is sent as 0, while right as size - 1
    if(result == -1)                                                //therefore for an array of size 10, 9 will be sent (since arrays are 0 indexed)
        cout<<"Element is not present in array";
    else
        cout<<"Element is present at "<<result<<"th position";
    return 0;
}
